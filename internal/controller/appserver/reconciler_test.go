...
var _ = Describe("App server reconciliator", Ordered, func() {
	Context("Creation logic", Ordered, func() {
		...
		It("should defer update when postgres not ready during CA rotation", func() {
			...
			By("Ensure deployment has initial CA and Secret hash annotations set (simulates established state)")
			...
			appDep.Annotations[utils.PostgresSecretHashKey] = "initial-secret-hash"
			appDep.Spec.Template.Annotations[utils.PostgresCAHashKey] = "initial-ca-hash"
			appDep.Spec.Template.Annotations[utils.PostgresSecretHashKey] = "initial-secret-hash"
			...
			By("Simulate Postgres CA hash change in state cache (secret hash unchanged)")
			testReconcilerInstance.GetStateCache()[utils.PostgresCAHashStateCacheKey] = "new-ca-hash-value"
			testReconcilerInstance.GetStateCache()[utils.PostgresSecretHashStateCacheKey] = "initial-secret-hash"

...
func TestBuildLlamaStackEnvVars_GenericProvider_CustomCredentialKey(t *testing.T) {
	const secretName = "fireworks-creds"
				...
				Providers: []olsv1alpha1.ProviderSpec{
					{
						...
						Config: &runtime.RawExtension{
							Raw: []byte(`{"url": "https://api.fireworks.ai/inference/v1"}`),
						},
						Models: []olsv1alpha1.ModelSpec{{Name: "accounts/fireworks/models/llama-v3-70b"}},
					},
				},
			},
		},
	...
	fakeSecret := &corev1.Secret{
		...
		Data: map[string][]byte{
			customKey: []byte("my-fireworks-token"),
		},
	}
	...
	// Find the FIREWORKS_API_KEY env var
	var apiKeyEnv *corev1.EnvVar
	for i := range envVars {
		if envVars[i].Name == "FIREWORKS_API_KEY" {
			apiKeyEnv = &envVars[i]
			break
		}
	}
	...
	if apiKeyEnv == nil {
		t.Fatal("FIREWORKS_API_KEY env var not found in generated env vars")
	}
	...
	if apiKeyEnv.ValueFrom == nil || apiKeyEnv.ValueFrom.SecretKeyRef == nil {
		t.Fatal("FIREWORKS_API_KEY env var does not reference a secret")
	}
...
func TestBuildLlamaStackEnvVars_GenericProvider_DefaultCredentialKey(t *testing.T) {
	const secretName = "together-creds"
				...
				Providers: []olsv1alpha1.ProviderSpec{
					{
						...
						Models: []olsv1alpha1.ModelSpec{{Name: "meta-llama/Llama-3-70b-chat-hf"}},
					},
				},
			},
		},
	...
	fakeSecret := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{Name: secretName},
		Data: map[string][]byte{
			"apitoken": []byte("my-together-token"),
		},
	}
	...
	var apiKeyEnv *corev1.EnvVar
	for i := range envVars {
		if envVars[i].Name == "TOGETHER_API_KEY" {
			apiKeyEnv = &envVars[i]
			break
		}
	}
	...
	if apiKeyEnv == nil {
		t.Fatal("TOGETHER_API_KEY env var not found")
	}

	if apiKeyEnv.ValueFrom == nil || apiKeyEnv.ValueFrom.SecretKeyRef == nil {
		t.Fatal("TOGETHER_API_KEY does not reference a secret")
	}

...
func TestBuildLlamaStackYAML_WatsonxProvider(t *testing.T) {
				...
				Providers: []olsv1alpha1.ProviderSpec{
					{
						...
						WatsonProjectID: "test-project-id",
						Models: []olsv1alpha1.ModelSpec{
							{
	...
	requiredFields := []string{
		"api_key",    // API key env var
		"project_id", // Watsonx project ID
		"url",        // Watsonx endpoint
	}
	for _, field := range requiredFields {
		if _, exists := config[field]; !exists {
	...
	if projectID, ok := config["project_id"].(string); ok {
		if projectID != "test-project-id" {
			t.Errorf("Expected project_id 'test-project-id', got '%s'", projectID)
		}
	} else {
	...
	if url, ok := config["url"].(string); ok {
		if url != "https://us-south.ml.cloud.ibm.com" {
			t.Errorf("Expected url 'https://us-south.ml.cloud.ibm.com', got '%s'", url)
		}
	} else {
		t.Errorf("url field is missing or invalid type")
	}

...
// TODO: Add tests for additional generic providers (e.g., fireworks-ai, together-ai)
...
func TestBuildLlamaStackYAML_GenericProvider_CustomCredentialKey(t *testing.T) {
	// Test that a generic provider with a custom credentialKey still generates valid
	// Llama Stack YAML with api_key auto-injected in the config section.
	// NOTE: credentialKey controls which key in the Kubernetes Secret is read during
	// deployment env-var wiring (buildLlamaStackEnvVars), NOT the config YAML here.
	// The actual SecretKeyRef.Key behaviour is covered by
	// TestBuildLlamaStackEnvVars_GenericProvider_CustomCredentialKey in deployment_test.go.
	cr := &olsv1alpha1.OLSConfig{
		Spec: olsv1alpha1.OLSConfigSpec{
			LLMConfig: olsv1alpha1.LLMSpec{
				Providers: []olsv1alpha1.ProviderSpec{
	...
	// api_key should be auto-injected into the YAML config section regardless of
	// credentialKey (credentialKey only affects which Secret key is read for the env var).
	apiKey, ok := config["api_key"]
	if !ok {
		t.Error("api_key not auto-injected into config YAML")
	} else {
	...
	t.Logf("✓ Generic provider YAML config generated correctly; credentialKey SecretKeyRef is tested in deployment_test.go")
}
